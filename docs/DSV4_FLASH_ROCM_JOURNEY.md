...
1. **稀疏索引重放的限制**:DSv4 的 DSA 索引重放(记录推理时注意力选了哪些 KV、训练时重放)在 sglang 里由一个 capturer 实现,它**要求 rollout 的注意力张量并行 = 1**(每张卡有完整注意力、DP 冗余算)。我们 rollout 用的是注意力 TP4,不满足,直接 assert 崩。解决:关掉 `--use-rollout-indexer-replay`(miles 侧)。
   - **为什么 NV 不撞这个**:大模型 rollout 通常跑"注意力 DP(attn_tp=1)+ 专家 EP"——注意力小、每卡放满一份;专家大、按 EP 切开。这样 attn_tp=1 天然满足 capturer 的要求。我们这套 rollout 用的是注意力 TP4,才撞上。所以这不是"NV 有 bug 我们没有",是**两边 rollout 的注意力并行方式不同**(可回头把我们 rollout 改成 attn_tp=1 对齐,见待办)。
...
| 维度 | NV 官方 | 我们 4 节点 |
|---|---|---|
| 注意力并行 | 推断 attn_tp=1(DP 注意力)+ EP 专家 | 注意力 **TP4** + EP4,8 引擎 |
| indexer-replay(DSA 版 R3) | 开(attn_tp=1 满足 capturer) | **关**(TP4 不满足;见第五节) |
