...
**backward kernel 对照(AMD vs NV):**

| 计算 | AMD(我们) | NV | 差别 |
|---|---|---|---|
| 激活重算 checkpoint | Megatron `te_checkpoint` | 同源 | 同 |
| dgrad GEMM(fp8) | TE **hipBLASLt** / blockwise-triton | TE **cuBLAS/cutlass** fp8 | 同源,后端不同 |
| wgrad GEMM(fp8) | TE hipBLASLt / blockwise-triton | TE cuBLAS/cutlass fp8 | 同源,后端不同 |
| **wgrad 累加融合** | **关** — ROCm TE 的 blockwise grouped-fp8(MoE)不支持 wgrad-accum fusion(抛 NotImplementedError)→ 关融合,wgrad 当普通梯度、DDP hook 单独 `add_` 进 fp32 main_grad | **开** — APEX `wgrad_gemm_accum_fp32` 直接把 wgrad 累加进 main_grad | ⚠️ **唯一实质差别** |
| 梯度 fp32 累积/规约 | fp32(`--accumulate-allreduce-grads-in-fp32`) | fp32 | 同 |
| 跨节点梯度规约 | RCCL over RDMA;DP=1 → 只 PP 激活梯度 p2p 出节点 | NCCL;同拓扑 | 同(RCCL=ROCm 版 NCCL) |
| 优化器 GPU 部分 | TE `FusedAdam`(HIP) | TE `FusedAdam`(CUDA) | 同源 |
| 优化器 offload 部分 | `HybridDeviceOptimizer`:75% 参数 host CPU `AdamW` + PCIe D2H/H2D | 同代码路径(NV 8 节点通常不需 offload) | 配置差别,非 arch |
| master remainder | TE `multi_tensor_adam_param_remainder`(HIP) | 同(CUDA) | 同源 |

> backward 侧 **AMD/NV 绝大多数同源**(TE + Megatron,只是 hipBLASLt vs cuBLAS 后端);**唯一实质差别是 wgrad 累加融合** —— ROCm TE 的 blockwise grouped-fp8 没实现,所以我们关掉融合、用 DDP hook 补一次 fp32 累加(数值等价、多一步),NV 保留融合。
