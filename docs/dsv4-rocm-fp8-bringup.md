...
- **E24** ✅(已绕,关 R3) generate 完→train step 撞 `ValueError: rollout_routed_experts is required in rollout_data for replay`(`actor.py train_actor`→`replay_data.py:53`)。`--use-rollout-routing-replay`(R3,`run_deepseek_v4.py enable_r3`)需 rollout 回传 MoE routed_experts(`sglang_rollout.py:178` 请求 `return_routed_experts`→收 `meta_info["routed_experts"]`),但 ROCm 拿不到 routing→缺。**真因(2026-06-28 verified,见下「性能优化实测结论」)**:miles 请求侧接线完整(`enable_return_routed_experts=True` 确实到 engine),sglang 的 topk capture hook(`select_experts`→`_post_process_topk_ids`)也平台无关、`RoutedExpertsCapturer` 照常创建——**但喂它的 producer 在 dsv4 aiter/ROCm topk 路径没接线**(和 `model_runner.init_indexer_capturer` 显式 CUDA-only guard 同一模式:"Producer wiring is CUDA-only ... other backends create a capturer but never feed it"),所以 capturer 创建了却捕不到,`meta_info["routed_experts"]` 为空→replay 缺。R3 是 on-policy 优化、sanity 不必需。修:`run_deepseek_v4.py __post_init__` `is_hip()` guard 关 `enable_r3`(NV 不影响)。**train44(2026-06-28 强开 R3 验证)实跑仍崩在此处,坐实 ROCm R3 受限**;启用需把 aiter topk producer 接到 capturer(deep sglang/aiter,超本阶段范围)。
...
sanity 验收后转「性能优先」,把上面三个待办(mhc / indexer / rollout)逐个**实测**,三个角度全撞 ROCm 墙或 toy 不 transfer;随后试 R3 严格 on-policy 也撞 ROCm 墙(routing producer 未接线,见下)。**结论:torch-fn 全链(rollout 3:04 + train step 110.5s + on-policy abs_diff 0.325)是当前 ROCm/4-layer 下的稳定绿配置;perf 三靶 + R3 均受限于 ROCm/toy,bring-up 阶段交付完成(端到端 fp8 训练 sanity 绿)。** 实测细节:
...
- **R3 严格 on-policy(待办②):试开后撞 ROCm 墙,已恢复 guard(R3 在 ROCm 关)。** spike 读代码以为可行(routing capture hook `select_experts`→`_post_process_topk_ids` 平台无关),强开 R3 跑 train44 验证 → **仍崩 `rollout_routed_experts is required`**(replay_data.py:53)。真因:miles 请求侧接线完整(`enable_return_routed_experts=True` 到 engine)、capturer 也创建,但**喂 capturer 的 producer 在 dsv4 aiter/ROCm topk 路径没接线**(同 `init_indexer_capturer` 显式 CUDA-only guard 模式)→ 捕不到 routing → meta_info 空 → replay 缺。启用需把 aiter topk producer 接到 capturer(deep sglang/aiter,超本阶段范围)。**教训复用**:连续多个「读代码以为成立」的假设(P1 indexer 越界、mhc 大头、R3 可行)实跑才证伪 → ROCm 路径必须实跑验证,别信纯代码推断。
...
### R3 严格 on-policy(见待办 ②,⚠️ 已实测撞 ROCm 墙)
- [x] **试开 R3 验证(train44,2026-06-28)→ 仍崩 `rollout_routed_experts is required`**:真因=routed_experts capture 的 producer 在 dsv4 aiter/ROCm topk 路径没接线(同 `init_indexer_capturer` 的 CUDA-only 模式),capturer 创建却捕不到。已恢复 `is_hip()` guard(R3 在 ROCm 关)。启用需把 aiter topk producer 接到 capturer(deep sglang/aiter,超本阶段范围)。详见上「性能优化实测结论」+ E24 更正。
