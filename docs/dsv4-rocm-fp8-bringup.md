...
| ⚠️ 重建坑 | 新镜像 `dev-20260629`(PR #1506)已 bake 掉大部分旧坑(click 8.2.1 / sglang fork `amd/dsv4-flash-rocm-rollout-20260703` / 对的 base aiter `7d604afe5`)。**仍需:① 别加 `--network host`**(同机别的容器跑 ray,host 网络下两个 ray head 撞 6379/8265 → `ray start --head` 失败)。**② `/root/miles` checkout `amd/dsv4-flash-rocm-enablement-20260701`**(build 默认 `MILES_COMMIT=main`,缺 actor 修 E2/E24/E25/E26;浅克隆 `git fetch --depth=1 <miles-wip> <branch>` + `checkout -B`)。env 不用手 export —— 训练脚本 `scripts/amd/run-deepseek-v4-flash-fp8-4layer-amd.sh` 自带全套。⚠️ 旧镜像 `dev-20260627` 另需手补 click/sglang fork,且 base aiter `a6bb49937` 缺 `fused_clamp_act_mul` → silu blocker(见 E16),**已被 dev-20260629 取代**。容器随 SLURM 24h 超时回收(`dsv4-fp8-v14/v15` 都这么没的),长跑前先 `salloc` 占节点。 |
| miles | 容器内 `radixark/main`;改动在 `XinyuJiangCMU/miles@amd/dsv4-flash-rocm-enablement-20260701` |
| sglang | fork `XinyuJiangCMU/sglang@amd/dsv4-flash-rocm-rollout-20260703`;Dockerfile fetch 该 branch |
...
- sglang 段 `git fetch ${SGLANG_REPO} ${SGLANG_BRANCH}` 拉 fork branch `amd/dsv4-flash-rocm-rollout-20260703`(= sglang-miles + E15 修),只装 python 层(不重编 sgl-kernel)。
...
- **E15** ✅(已解,fork+PR) MoE down_proj 收 fused_clamp fp8 tuple `(x_fp8,x_scale)`;sglang-miles 的 true_on_policy(sglang upstream PR 26359)给 RowParallelLinear 加了 `should_use_tp_invariant_row_linear(input_parallel.shape[-1])`,在 tuple 上读 `.shape` 炸(sglang main 无此模块)。根因:`matmul_tp_inv` 仅 bf16/fp16/fp32,fp8 row-linear 必走 `quant_method.apply`(`Fp8LinearMethod` 本就拆 tuple)。修:`if not isinstance(self.quant_method, Fp8LinearMethod) and should_use_tp_invariant_row_linear(...)`(读 .shape 前短路)。**fork+pin(非 patch):** [`XinyuJiangCMU/sglang@amd/dsv4-flash-rocm-rollout-20260703`](https://github.com/XinyuJiangCMU/sglang/tree/amd/dsv4-flash-rocm-rollout-20260703),PR(branch amd/dsv4-flash-rocm-rollout-20260703)(commit 链:tuple-skip → Fp8LinearMethod gate → torch fn path 3 修);Dockerfile fetch 该 branch。
...
- [x] **R3 价值验证 A/B + abs_diff 3.6 定性(loop R49–R55)**:修通后做 R3 on/off logprob-diff A/B(两臂同 miles router、只 toggle routing replay,隔离 router confound)。结果 **R3-ON `train_rollout_logprob_abs_diff` 3.66/3.56/3.64 ≈ R3-OFF 3.63/3.63 → R3 在 4-layer toy 上没把 on-policy 散度压下来**。诊断(2 agent + 实测):
  ...
  - ④ **⚠️ 更正本文档 line 100 的 "on-policy abs_diff 0.32"**:那是旧容器 train39(v14/v15,log 已删)、**不可复现、不能当基准**;现 v17 可复现基线 = 3.6。3.6 nats=概率差 36×=**结构性偏差非舍入**(舍入只 ~0.01-0.3),最可能 FP8 train(TE blockwise) vs sglang FP8 推理路径差 + DSA 稀疏注意力 train/rollout 选不同 top-k KV。**（后者已由下「indexer-replay」节实测证伪:两栈 top-k 确实差 20–42%,但强制对齐后 abs_diff 不动 → DSA 选择差非 3.6 来源;剩 FP8 路径差待 BF16 对照。）**
...
### indexer-replay(DSA KV top-k on-policy)✅ 跑通 + 实证(loop 2026-07-01)
- [x] **接入(loop R56–R100)**:R3 只回放 MoE 专家选择,indexer-replay 回放的是 DSA indexer 每 query 选的 top-k KV 位置(rollout sglang 端 `IndexerTopkCapturer` 捕获,train Megatron 重算 logprob 时回放,把两栈的 KV 选择对齐)。三处坑逐一啃通:**① host-buffer 死结** —— sglang `IndexerTopkCapturer` 的 host-pinned buffer 按 `max_total_num_tokens × num_layers × index_topk(512)` 预分配、远大于实际 capture 的 token 数,而 `max_total_num_tokens` 又同时决定 device KV pool,于是 `--sglang-mem-fraction-static` 大→host-pinned OOM(0.1→262GB 撞 BaseHostCache)、小→model 装不下(0.01→`Not enough memory`),同绑一个量、无解窗口。解法=用 `--sglang-max-total-tokens` 直接 cap `max_total_num_tokens`(`model_runner_kv_cache_mixin._apply_token_constraints` 的 `min(capacity, user_limit)`,而 `init_indexer_capturer` 的 host buffer 正用它),mem-frac 保 0.7 装 model;toy 设 1M(host buffer 降到 ~8–16GB ≈ 已知安全的 routed 量级)。**⚠️ max-total-tokens 1M 是 toy 权宜**(gate `"4layer" in model_name`,真模型 / NV 不受影响),根本该上游把 capturer host buffer 按**实际 capture token** 预分配、而非按 KV-pool 容量 ×512。**② reshape 2D 约定** —— replay manager 与 capture 用 2D `[n_tokens, kv]` 约定(MoE router 天然 2D、故 R3 无碍),而 V4Indexer 的 `index_scores` 是 3D `[batch, seqlen, kv]`,train 重算(序列打包 batch=1)`scores.shape[0]=1 ≠ replay 5120` → `replay_base._get_replay_result` assert `replay n_tokens != scores`。修:`v4_indexer.py` topk_fn 前 flatten 成 2D、后 reshape 回 `[batch, seqlen, index_topk]`(等价 `flashinfer_dsa_topk` 内部逻辑,非 replay path no-op-equivalent、NV 安全)。**③ capturer TP 限制** —— `state_capturer/indexer_topk.py` 原 `assert attn_tp_size==1`(DP-only);indexer top-k 是 per-query head-independent、attn-TP 各 rank 冗余算 full-batch,去掉 assert + `SGLANG_DSA_TOPK_BROADCAST=1`(rank0 广播 top-k)即可 TP 捕获(本 toy TP=4)。端到端 r89:rollout 256/256 无崩、log_probs 48.3s、step0 train step 202.3s、`train_rollout_logprob_abs_diff=3.5899`。miles 改 4 文件(`v4_indexer.py` / `deepseek_v4.py` / `run_deepseek_v4.py` / amd 脚本);sglang 改 `state_capturer/indexer_topk.py` 去 assert(diff 备份 `docs/sglang-indexer-topk-tp-capture.patch`,待下次 image rebuild 落 fork `amd/dsv4-flash-rocm-rollout-20260703`;运行容器已 live)。
- [x] **对抗验证 replay 真生效(loop R94)**:3 agent 压测——`_get_replay_result` 只在 stage=replay_forward/backward 被调(fallthrough 直接走 old_topk_fn),且这套 `get_topk_fn / register_to_module / pop_*` 在 deepseek_v4 物理上仅 indexer 走(R3 routing 走另一套 `register_replay_list_moe`),故那条 assert 只可能由 indexer 触发 = replay stage 确活;reshape 后 `_get_replay_result` 返回的是 replayed indices(非 old_topk_fn)、无 fallback。→ replay 真被应用,非静默 no-op。
- [x] **overlap 实证(loop R95–R98)**:临时开 `enable_check_replay_result` 量 rollout-vs-train 重算的 indexer top-k 重合度,`mean_overlap_ratio=0.804`(按层 0.58~0.999)。**两个结论**:① 非 ≈0 → reshape token 顺序**经验证实正确**(顺序错则 replayed index 对到错 token、overlap 会塌到 ~0),不只是"不崩";② 非均匀 1.0 → 两栈 indexer 选择**差异真实**(mean ~20%、最坏层 42% 的 512 KV pick 两栈选得不同,即便 step0 同权重)。
- [x] **abs_diff 结论(实证,更正 point ④ 的 DSA 猜测)**:indexer-replay ON(r89=3.5899)落在历史 OFF 组噪声带(3.54–3.71,含 r56=3.5415、r49 连 R3 也 off=3.6319)**正中、不可区分**。结合 overlap:两栈 indexer 选择明明差 20–42%,强制对齐(replay)却**没动 abs_diff** → **DSA indexer top-k 选择差异不是 3.6 的实质来源**,证伪 point ④ "DSA 稀疏注意力选不同 top-k KV" 那半个猜测。3.6 仍是两套独立 forward 栈(Megatron 重算 vs sglang 推理)+ 4-layer toy 放大的**结构性地板**,indexer 对齐 / R3 / 换的 4 kernel 均不实质影响它,±0.1 是 rollout 数据噪声;真要压 abs_diff 仍需 point ⑤ 的 BF16 全链对照 / 全模型。
