...
`docker/Dockerfile.rocm` is the ROCm counterpart (build-args `GPU_ARCH` + a ROCm `SGLANG_IMAGE_TAG`). Like the CUDA Dockerfile it installs prebuilt wheels instead of compiling the heavy bits: `WHEELS_REPO` + `WHEELS_TAG_ROCM` name a release of ROCm wheels (Transformer Engine + flash-attn, built per ROCm minor × gfx arch). ROCm is single-arch x86 with the GPU arch chosen by `GPU_ARCH`, so `build.py` picks each `rocm-*` variant's `WHEELS_TAG_ROCM` by gfx (gfx950 → MI35x, gfx942 → MI30x); the Dockerfile downloads that release's `.whl` assets and `pip install`s them. Keeping the tag→variant mapping in `build.py` (never in the Dockerfile) is the same boundary as the CUDA `WHEELS_TAG_*`. The download step fail-fasts: it asserts `WHEELS_TAG_ROCM` matches `GPU_ARCH`, uses `curl -f` on the release API, and errors if no `.whl` landed — so a missing release or arch/tag mismatch fails loudly instead of surfacing later as a confusing `pip` error.

> **Status:** only the **gfx950 / `rocm-mi350`** wheels release exists today (`rocm700-gfx950-v0.5.14`). The **gfx942 / `rocm-mi300`** wheels have not been built/published yet, so `rocm-mi300` cannot be built until its release is created.
