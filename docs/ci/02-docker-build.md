...
The ROCm rows describe what `build.py` would write. In practice the published ROCm images do not come from this workflow: they are built nightly from `radixark/miles@main` by the AMD release workflows in `sgl-project/sglang`, which call the same `build.py` with `--image-tag custom --custom-tag miles` and then push a dated tag. So `rocm/sgl-dev` carries `miles-<variant>` and `miles-<variant>-<YYYYMMDD>`, and no `latest-<variant>` tag has ever been published there.
