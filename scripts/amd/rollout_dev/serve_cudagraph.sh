...
set -e
# Default to the rocm720 TRAINING image (same as 4/6/8/9) so cuda-graph results transfer.
...
docker exec -d "$NAME" bash -lc '
  ...
  python -m sglang.launch_server \
    ...
    > /workspace/rollout_dev/serve_cudagraph.log 2>&1
'
echo "server launching on $(hostname) GPUs $GPUS port $PORT (miles src: $MILES_SRC)"
echo "watch:  tail -f /mnt/data/data/hai/rollout_dev/serve_cudagraph.log"
