...
def _to_local_gpu_id(physical_gpu_id: int) -> int:
    # On AMD/ROCm, CUDA_VISIBLE_DEVICES may not be set; check HIP_VISIBLE_DEVICES too
    cvd = os.environ.get("CUDA_VISIBLE_DEVICES") or os.environ.get("HIP_VISIBLE_DEVICES")
    if not cvd:
        return physical_gpu_id  # no remapping
    # CUDA_VISIBLE_DEVICES / HIP_VISIBLE_DEVICES can be like "4,5,6,7"
    ...
    raise RuntimeError(
        f"GPU id {physical_gpu_id} is not valid under visible devices={cvd}. "
