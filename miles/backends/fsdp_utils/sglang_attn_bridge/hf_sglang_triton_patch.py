"""Replace HF attention kernel with SGLang Triton extend_attention_fwd_unified.

We register a custom attention backend via HF's ALL_ATTENTION_FUNCTIONS. HF keeps its
own qkv_proj, norm, rope, o_proj logic — we only replace the core attention computation
(Q@K^T softmax V) with extend_attention_fwd_unified.

The extend_attention_fwd_unified kernel uses per-request indptrs, which gives
batch-invariant results (each request is computed independently).
"""
...
def _sglang_triton_attention(
    ...
    **kwargs,
    ...
    return o.view(B, S, num_heads, D), None
